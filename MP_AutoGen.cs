...
public class MP_AutoGen : MonoBehaviour
{
    ...
    Dropdown rankDrop;
    [SerializeField]
    GameObject resultsWindow, resetBut, resultsDex, inputDex;
    [SerializeField]
    Text[] resultObjs;
    [SerializeField]
    Text[] resultAdvs;
    ...
    string nature, monName;
    int rank, typeOne, typeTwo, monNum;
    string[] types;
    MP_PokeList pL;
    MP_PokemonMoves pM;
    List<string> knownMoves = new List<string>();

    // Start is called before the first frame update
    void Start()
    {
        pL = GetComponent<MP_PokeList>();
        atts = new int[5];
        attLimits = new int[5];
        social = new int[5];
    ...
    {
        string temp = inputObjs[0].text.Substring(0, 1).ToUpper() + inputObjs[0].text.Substring(1, inputObjs[0].text.Length - 1).ToLower();

        for(int i = 0; i < pL.GetPoke().Length; i++)
        {
            if (temp == pL.GetPoke()[i].pokeName)
            {
                SubmitStats(i);
                break;
            }
    ...
    public void FindDex()
    {
        for (int i = 0; i < pL.GetPoke().Length; i++)
        {
            if (System.Convert.ToInt32(inputObjs[1].text) == pL.GetPoke()[i].dexNum)
            {
                SubmitStats(i);
                break;
            }
    ...
    public void SubmitStats(int pokemon)
    {
        Debug.Log(pokemon);
        monName = pL.GetPoke()[pokemon].pokeName;
        monNum = pL.GetPoke()[pokemon].dexNum;

        atts[0] = pL.GetPoke()[pokemon].str;
        attLimits[0] = pL.GetPoke()[pokemon].strLmt;
        atts[1] = pL.GetPoke()[pokemon].dex;
        attLimits[1] = pL.GetPoke()[pokemon].dexLmt;
        atts[2] = pL.GetPoke()[pokemon].vit;
        attLimits[2] = pL.GetPoke()[pokemon].vitLmt;
        atts[3] = pL.GetPoke()[pokemon].spl;
        attLimits[3] = pL.GetPoke()[pokemon].splLmt;
        atts[4] = pL.GetPoke()[pokemon].ins;
        attLimits[4] = pL.GetPoke()[pokemon].insLmt;

        HP = pL.GetPoke()[pokemon].hp;

        typeOne = pL.GetPoke()[pokemon].typeOne;
        typeTwo = pL.GetPoke()[pokemon].typeTwo;
        rank = rankDrop.value;

        /*pM = new MP_PokemonMoves(pL.GetMonMoves()[pokemon].starter, pL.GetMonMoves()[pokemon].beginner, pL.GetMonMoves()[pokemon].amateur, pL.GetMonMoves()[pokemon].ace, pL.GetMonMoves()[pokemon].pro);*/

        GetResults();
    }
    public void RandPoke()
    {
        int pokemon = Random.Range(0, 781);
        SubmitStats(pokemon);
    }
    ...
    void SetMoves()
    {
        int moveNum = atts[4] + 2;
        List<int> possibleMoves = new List<int>();

        if (rank >= 0)
            possibleMoves.AddRange(pM.starter);
        if (rank >= 1)
            possibleMoves.AddRange(pM.beginner);
        if (rank >= 2)
            possibleMoves.AddRange(pM.amateur);
        if (rank >= 3)
            possibleMoves.AddRange(pM.ace);
        if (rank >= 4)
            possibleMoves.AddRange(pM.pro);

        int[] posMoves = possibleMoves.ToArray();
        if (moveNum >= possibleMoves.Count)
        {
            for(int i = 0; i < posMoves.Length; i++)
            {
                knownMoves.Add(pL.GetMoves()[posMoves[i]]);
            }
        }
        else
        {
            ...
            while (x > 0)
            {
                int m = Random.Range(0, possibleMoves.Count);
                knownMoves.Add(pL.GetMoves()[posMoves[m]]);
                possibleMoves.RemoveAt(m);
                posMoves = possibleMoves.ToArray();
                x--;
            }
        }
    }
    void SetResults()
    {
        ...
        resultObjs[25].text = "" + monName;
        resultObjs[26].text = "#" + monNum;

        /*
        string moveResult = "";
        foreach(string move in knownMoves)
        {
            moveResult += move.ToString() + "\n";
        }
        resultObjs[27].text = moveResult;*/
    }
    void SetRanks()
    {
        ranks[0] = "Starter";
